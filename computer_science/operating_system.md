...
2. ### [커널(kernel)](#2-커널)
   ...
   - #### [커널/커널 오브젝트의 의미](#2-1-커널의-의미)
   - #### [핸들의 의미](#2-2-핸들의-의미)
   - #### [커널오브젝트의 특징/핸들의 특징](#2-3-커널오브젝트와-핸들의-특징)
   
3. ### [쓰레드(thread)](#3-쓰레드)

   - #### [쓰레드의 의미](#3-1-쓰레드의-의미)
   
   - #### [프로세스와 쓰레드의 차이](#3-2-프로세스와-쓰레드의-차이)
...
메모리 할당이 이루어지고 **실행중인 프로그램**을 의미. 즉 메인메모리에서 실행되고 있는 프로그램.
...
**커널**이란 컴퓨터를 운영하는 데 있어서 중심이 되는 **운영체제 핵심 부분**

**커널 오브젝트**는 프로세스, 쓰레드, 파일과 같은 리소스들을 원할하게 관리하기 위해 필요한 정보를 저장해야 하는데, 이때 **데이터를 저장하는 메모리 블록을 커널 오브젝트**라고 한다

</br>

> ## 2-2. 핸들의 의미

**핸들**이란 **커널오브젝트를 간접적으로 변경**시키기 위해 **커널 오브젝트에 할당되는 숫자**

</br>

> ## 2-3. 커널오브젝트와 핸들의 특징

**커널 오브젝트**는 **운영체제에 종속적**이다. 따라서, **커널 오브젝트의 소멸 시점은 운영체제에 의해 결정**된다. 여러 프로세스에 의해 접근이 가능하다.
...
- **프로세스**
  ...
  데이터에 관해 서로 완벽히 독립적인 구조이다. 

- **쓰레드**

  쓰레드는 같은 프로세스 내에서 **서로 공유하는 요소들**이 있기 때문에 컨텍스트 스위칭이 프로세스보다 빠르다.

</br>

### 쓰레드끼리 공유하는 요소

메모리 구조상 프로세스는 code영역, data영역, stack영역, heap영역으로 구성된다.

쓰레드는 쓰레드가 생성될 때마다 **해당 쓰레드를 위한 stack영역을 만들고** 나머지 **code, data, stack영역은 공유**하고 있습니다.

</br>

### 커널 레벨 쓰레드 / 유저 레벨 쓰레드

**커널 레벨 쓰레드**란, **쓰레드를 생성하고 스케줄링 하는 주체가 커널**일 경우를 뜻하고

**유저 레벨 쓰레드**란, 커널에서 쓰레드 기능을 지원하지 않을 때, **커널에 의존하지 않고 라이브러리를 활용하여 쓰레드의 기능을 써서 실행되는 쓰레드**를 뜻한다.

**커널 레벨 쓰레드**는 **커널에서 직접 관리해주는 쓰레드**이기 때문에 안전성이 뛰어나고 기본으로 제공되는 기능들이 있다.

**유저 레벨 쓰레드**는 커널에 의존하지 않기 때문에 커널모드와 유저모드의 전환이 없어서 성능이 좋다.

</br>

### 커널 영역 / 유저 영역

**커널 영역**은 **운영체제** 동작시 사용하는 메모리 공간을 뜻하고,

**유저 영역**은 사용자가 원하는 프로그램 동작시 사용되는 메모리 공간(**code, data, stack, heap**)을 뜻한다.
...
**유저 모드**는 유저 영역에서 실행이 이뤄져야 할 경우 전환되는 모드이다.

**커널 모드**에서는 모든 영역에 대한 접근이 가능하다.

**유저 모드**에서는 커널 영역에 대한 접근이 불가능하다.
